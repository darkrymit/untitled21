...
def show_table_1(r, z, t, title) :
    ...
    ax = plt.subplot()
    columns = ['ri', 'zi', 'ti']
    the_table = ax.table(cellText = data_list,
                         colLabels = columns,
                         cellLoc = 'center',
                         loc = 'center')
...
def show_plot() :
    if __name__ == '__main__' :
        ...
        T_min = 0
        r = [round(random.uniform(0, 1), 4) for i in range(1, 1000)] # випадкові рівномірно розподілені числа від 0 до 1
        ...
        xj1 = [len(x1[j]) for j in range(len(x1))] # кількість викликів, що потрапили у проміжок часу довжиною t = 1 хв.
